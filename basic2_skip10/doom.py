...
with tf.Session() as sess:
      ...
      # run
      deep_q_learning(sess, game, q_net=q_net, target_net=target_net, state_processor=state_processor, num_episodes=10000,
                            train_or_test=train_or_test, train_from_scratch=train_from_scratch, start_iter=start_iter, start_episode=start_episode,
                                    replay_memory_size=100000, replay_memory_init_size=5000, update_target_net_every=3000,
                                    gamma=1.0, epsilon_start=epsilon_start, epsilon_end=[0.1,0.001], epsilon_decay_steps=[1e4,1e4], batch_size=64)
